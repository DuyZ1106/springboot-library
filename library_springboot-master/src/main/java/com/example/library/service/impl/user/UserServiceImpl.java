...
public class UserServiceImpl implements UserService {
    ...
    public Page<UserResponse> searchUsers(UserSearchRequest request, Pageable pageable) {
        Specification<User> spec = Specification.where((root, query, cb) -> cb.isFalse(root.get("isDeleted")));

        if (hasText(request.getUsername())) {
            spec = spec.and((root, query, cb) -> cb.like(cb.lower(root.get("username")), "%" + request.getUsername().toLowerCase() + "%"));
        }
        if (hasText(request.getFullName())) {
            spec = spec.and((root, query, cb) -> cb.like(cb.lower(root.get("fullName")), "%" + request.getFullName().toLowerCase() + "%"));
        }
        ...
        if (hasText(request.getAddress())) {
            spec = spec.and((root, query, cb) -> cb.like(cb.lower(root.get("address")), "%" + request.getAddress().toLowerCase() + "%"));
        }
        if (request.getAge() != null) {
            spec = spec.and((root, query, cb) -> cb.equal(root.get("age"), request.getAge()));
        }
        if (request.getBirthday() != null) {
            spec = spec.and((root, query, cb) -> cb.equal(root.get("birthday"), request.getBirthday()));
        }
